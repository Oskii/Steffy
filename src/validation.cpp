...
#if defined(NDEBUG)
# error "Steffy cannot be compiled without assertions."
#endif
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs)
{
    ...
    if (tx.IsCoinBase())
    {
        ...
        std::string developerWallet = "KT9BpWdBbbSLKD3fQBv64hK4kyyMpLektS";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s STEF additional fees, %d delta bytes\n",
